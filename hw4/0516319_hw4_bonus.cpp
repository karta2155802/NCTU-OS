...
sem_t s_mean[1861][2631], s_grey[1861][2631];
...
void* MeanFilter(void* args)
{
    ...
    for(h=0;h<imgHeight;h++)
        for(w=0;w<imgWidth;w++)
        {
            if(h == imgHeight-1 && w == imgWidth-1)sem_wait(&s_grey[h][w]);
            else if(w == imgWidth-1)sem_wait(&s_grey[h+1][w]);
            else if(h == imgHeight-1)sem_wait(&s_grey[h][w+1]);
            else sem_wait(&s_grey[h+1][w+1]);
            int tmp = 0;
            ...
            if(h == imgHeight-1 && w == imgWidth-1)sem_post(&s_grey[h][w]);
            else if(w == imgWidth-1)sem_post(&s_grey[h+1][w]);
            else if(h == imgHeight-1)sem_post(&s_grey[h][w+1]);
            else sem_post(&s_grey[h+1][w+1]);
            sem_post(&s_mean[h][w]);
            sem_post(&s_mean[h][w]);
        }
}
...
int main()
{
    ...
    for (int k = 0; k<5; k++){
        ...
        pthread_create(&th2, NULL, gx_sobelFilter, NULL);
        pthread_create(&th3, NULL, gy_sobelFilter, NULL);
